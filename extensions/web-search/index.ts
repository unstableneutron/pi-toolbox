 ...
 *
 * Uses the Parallel SDK when PARALLEL_API_KEY is available, with
 * anonymous Parallel Search MCP fallback when no key is configured or
 * the keyed account cannot be used.
 */
...
export function shouldFallbackToMcp(err: unknown): boolean {
  ...
  return message.includes('insufficient credit') || message.includes('billing');
}
  ...
  tools.push(
    defineTool({
      ...
      async execute(_toolCallId, params, signal, onUpdate) {
        ...
        const fallbackReason = sdkDisabledReason ?? (sdkClient ? 'sdk_unavailable' : undefined);
        const search = await executeWithFallback<SearchResponse>({
          sdk: sdkClient
            ? async () => {
                try {
                  const response = await sdkClient.beta.search(
                    {
                      objective,
                      search_queries,
                      mode: 'agentic',
                  ...
                  return { ...(response as SearchResponse), provider: 'sdk' as const };
                } catch (err) {
          ...
          mcp: async () => {
            ...
            return { ...response, provider: 'mcp' as const, fallbackReason };
          },
      ...
      async execute(_toolCallId, params, signal, onUpdate) {
        ...
        const extract = await executeWithFallback<ExtractResponse>({
          sdk: sdkClient
            ? async () => {
                try {
                  const response = await sdkClient.beta.extract(
                    {
                      urls,
                      objective,
                      excerpts: true,
                  ...
                  return { ...(response as ExtractResponse), provider: 'sdk' as const };
                } catch (err) {
                  if (signal?.aborted || (err instanceof Error && err.name === 'AbortError')) {
