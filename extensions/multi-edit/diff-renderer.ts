...
import { pluralize, sanitizeAnsiForThemedOutput } from './render-utils';
...
const ANSI_RGB_PATTERN = new RegExp(String.raw`\\u001b\[(?:3|4)8;2;(\d{1,3});(\d{1,3});(\d{1,3})m`);
const ANSI_256_PATTERN = new RegExp(String.raw`\\u001b\[(?:3|4)8;5;(\d{1,3})m`);
...
function parseDiff(diffText: string): ParsedDiff {
  ...
  for (const rawLine of diffText.replace(/\r/g, '').split('\n')) {
    ...
    if (rawLine.startsWith('diff --git ') || rawLine.startsWith('File: ')) {
      stats.files++;
      oldLineCursor = null;
      newLineCursor = null;
      lineNumberDelta = 0;
...
function parseAnsiColorCode(ansi: string | undefined): RgbColor | null {
  ...
  const rgbMatch = ANSI_RGB_PATTERN.exec(ansi);
  if (rgbMatch) {
  ...
  const bitMatch = ANSI_256_PATTERN.exec(ansi);
  if (bitMatch) {
...
function renderSplit(
  ...
  for (const row of rows) {
    ...
    for (let index = 0; index < rowCount; index++) {
      const leftCell =
        leftCells[index] ?? renderSplitBlankCell(leftWidth, splitLineNumberWidth, theme);
      const rightCell =
        rightCells[index] ?? renderSplitBlankCell(rightWidth, splitLineNumberWidth, theme);
...
function renderWriteHeader(wasOverwrite: boolean, width: number, theme: DiffTheme): string {
...
export function renderWriteDiffResult(
  ...
  return {
    render(width: number): string[] {
      ...
      const header = renderWriteHeader(options.fileExistedBeforeWrite === true, safeWidth, theme);
      if (overwriteGuard) {
