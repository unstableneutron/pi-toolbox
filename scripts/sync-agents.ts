#!/usr/bin/env bun
/**
 * Sync subagent definitions between
 *
 *   repo: <pi-toolbox>/agents/
 *   live: ~/.pi/agent/agents/
 *
 * `agents/` is NOT a standard pi-package convention directory (pi supports
 * extensions/, skills/, prompts/, themes/). `pi-subagents` loads user agents
 * from `~/.pi/agent/agents/` and `~/.agents/`, so installing pi-toolbox as a
 * package does not expose the repo's agents/*.md files to the agent host. We
 * sync them here the same way `sync-extension-configs.ts` syncs
 * extensions-config/.
 *
 ...
 *
 *   bun run scripts/sync-agents.ts           # dry-run
 *   bun run scripts/sync-agents.ts --apply   # interactive
 */
...
const REPO_DIR = join(REPO_ROOT, 'agents');
const LIVE_DIR = join(homedir(), '.pi', 'agent', 'agents');
...
function classify(): FilePair[] {
  ...
  for (const repoPath of listFiles(REPO_DIR)) {
    ...
    // Skip READMEs; they're documentation, not synced agent definitions.
...
function unifiedDiff(
  ...
  // Naive line-by-line diff; sufficient for short agent markdown files.
...
function printStatus(pairs: FilePair[]): void {
  console.log(`${BOLD}sync-agents${RESET}`);
